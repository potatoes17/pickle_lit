...
def get_gsheet():
    credentials = Credentials.from_service_account_file(
        "your-key-file.json",  # <-- Replace this on Streamlit Cloud
        scopes=SCOPE
    )
...
if st.button("🔄 Scrape and Update Database"):
    ...
    worksheet.batch_update([{
        'range': f"A2:I{len(book_data)+1}",
        'values': book_data
    }])
    st.success(f"✅ {len(book_data)} books scraped and saved to Google Sheets!")
